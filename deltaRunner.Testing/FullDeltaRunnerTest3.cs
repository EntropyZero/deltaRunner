...
{
	[TestFixture]
	public class FullDeltaRunnerTest3 : TestFixtureBase
	{
		...
		public void TearDown()
		{
            ...
            if(File.Exists(Path.Combine(deltaPath, "00004aa.sql")))
            {
                File.Delete(Path.Combine(deltaPath, "00004aa.sql"));
            }            
            if(File.Exists(Path.Combine(deltaPath, "00004ab.sql")))
            {
                File.Delete(Path.Combine(deltaPath, "00004ab.sql"));
            }            
            if(File.Exists(Path.Combine(deltaPath, "00004ba.sql")))
            {
                File.Delete(Path.Combine(deltaPath, "00004ba.sql"));
            }
		}
		...
		[Test]
		public void Execute_CheckingForNewDeltas_WithLetters()
		{
			RunSqlScript(new StreamReader(databasePopulateFilename), connectionString);
            DeltaRunnerBase deltaRunner = null;
			deltaRunner = DeltaRunnerFactory.CreateDeltaRunner(connectionString, deltaPath, true);
			...
			using (StreamWriter sw = new StreamWriter(Path.Combine(deltaPath, "00004aa.sql")))
			{
				sw.WriteLine("INSERT INTO Author ([Lastname], [Firstname]) VALUES('TestLastName', 'TestFirstName')");
				sw.WriteLine("GO");
				sw.Close();
			}			
            using (StreamWriter sw = new StreamWriter(Path.Combine(deltaPath, "00004ab.sql")))
			{
				sw.WriteLine("INSERT INTO Author ([Lastname], [Firstname]) VALUES('TestLastName', 'TestFirstName')");
				sw.WriteLine("GO");
				sw.Close();
			}			
            using (StreamWriter sw = new StreamWriter(Path.Combine(deltaPath, "00004ba.sql")))
			{
				sw.WriteLine("INSERT INTO Author ([Lastname], [Firstname]) VALUES('TestLastName', 'TestFirstName')");
				sw.WriteLine("GO");
				sw.Close();
			...
			Assert.AreEqual(6, ExecuteScalar("SELECT Count(*) FROM Author WHERE [Lastname] = 'TestLastName'", connectionString));
            SqlDeltaRunner.DropDatabase("DeltaRunner_FullTest3", connectionStringMaster);
            RunSqlScript(new StreamReader(databaseCreateFilename), connectionStringMaster);
            RunSqlScript(new StreamReader(databasePopulateFilename), connectionString);
            deltaRunner.PrepareForDeltaRunner();
            deltaRunner.OnAfterScriptExecution += new ScriptExecution(deltaRunner_OnAfterScriptExecution_Lookingforletters);
            deltaRunner.ApplyDeltas();
            Assert.AreEqual("00007", deltaRunner.GetLatestVersion());
        }

	    private string _lastDelta = null;
        void deltaRunner_OnAfterScriptExecution_Lookingforletters(ScriptExecutionArgs e)
        {
            switch(_lastDelta)
            {
                case null :
                    Assert.AreEqual("00001.sql", e.Delta.Filename);
                    break;
                case "00001.sql" :
                    Assert.AreEqual("00002.sql", e.Delta.Filename);
                    break;
                case "00002.sql" :
                    Assert.AreEqual("00003.sql", e.Delta.Filename);
                    break;
                case "00003.sql" :
                    Assert.AreEqual("00004.sql", e.Delta.Filename);
                    break;
                case "00004.sql" :
                    Assert.AreEqual("00004aa.sql", e.Delta.Filename);
                    break;
                case "00004aa.sql" :
                    Assert.AreEqual("00004ab.sql", e.Delta.Filename);
                    break;
                case "00004ab.sql" :
                    Assert.AreEqual("00004ba.sql", e.Delta.Filename);
                    break;
                case "00004ba.sql" :
                    Assert.AreEqual("00005.sql", e.Delta.Filename);
                    break;
            }
            _lastDelta = e.Delta.Filename;
        }
